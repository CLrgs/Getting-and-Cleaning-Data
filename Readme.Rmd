...
1.b) merge "test" & "train" to one file named "data" (with rbind)
convert first 2 collumns to factors
Now we have one file ("data") with 
- all rows/observations (10299 = 7352 + 2947) and 
- 563 collumns/variables/features : 1 col. for subjects, 1 col. for activity and 561 collumns for features

2) Extracts only the measurements on the mean and standard deviation for each measurement.
2.a) take/read the file "feature" and create a dataframe (file = features1 ) containing only mean & sd features. 
features1 <- features[grep("[M,m]ean|[S,s]td", features$V2), ]
The dataframe has 2 collumns: the 1st with the Nr. of variable/feature (i.e.: 1, 2 etc) and the 2nd with the full name of the variable/feature (i.e.: tBodyAcc-mean()-X, tBodyAcc-mean()-Y etc). There are 86 such variables(rows) in the dataframe.
2.b) choose from "data" (created in previous step Nr.2) the  first two collumns (= "Subjects" + "Activity") plus all collumns matching vector features1$V1 (i.e.: the number of the variable) and pass them to a new data frame : "data1"
...
"Activity" is a factor with 6 levels 1:6. We rename the labels taking the new names from the file "activity_labels.txt"
